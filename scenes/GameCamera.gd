...
func _process(delta):
	...
	if(currentShakePercentage > 0):
		...
		var xSample = shakeNoise.get_noise_2d(xNoiseSamplePosition, xNoiseSamplePosition.y)
		var ySample = shakeNoise.get_noise_2d(yNoiseSamplePosition, yNoiseSamplePosition.y)
		
		var calculateOffset = Vector2(xSample, ySample) * maxShakeOffset * pow(currentShakePercentage, 2);
